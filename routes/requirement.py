...
CORS(requirements_bp, origins=["http://localhost:3000", "http://localhost:8080", "http://127.0.0.1:3000"], supports_credentials=True)
...
def get_requirements():
    ...
    result = [req.to_dict() for req in requirements]  # Using to_dict() for serialization
...
def update_requirement(id):
    ...
    requirement.resource_requirement = data.get('resource_requirement', requirement.resource_requirement)
    requirement.job_description = data.get('job_description', requirement.job_description)
